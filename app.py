...
st.set_page_config(
    page_title="StockScope AI Pro V3.1",
    page_icon="📈",
    layout="wide",
...
st.title("📈 StockScope AI Pro V3.1")
...
def coingecko_market_chart(coin_id, days=365):
    ...
    try:
        ...
        r = requests.get(
            ...
            timeout=12,
            headers={"User-Agent": "StockScopeAI/3.1"},
        )
        r.raise_for_status()
        data = r.json()
...
def add_features(df):
    ...
    # Some crypto/data fallbacks return incomplete or zero volume. Treat that as
    # "volume signal unavailable" instead of allowing NaNs to crash the model.
    vol = pd.to_numeric(x.get("Volume", 0.0), errors="coerce").replace(0, np.nan)
    if isinstance(vol, pd.Series) and vol.notna().sum() >= 5:
        x["VolumeChange"] = vol.pct_change()
        x["VolumeRatio20"] = vol / vol.rolling(20).mean()
    else:
        x["VolumeChange"] = 0.0
        x["VolumeRatio20"] = 1.0
...
def make_forecast_cached(model_hist, days, asset_type):
    """Robust ML ensemble with imputation and a statistical fallback."""
    feat = add_features(model_hist).replace([np.inf, -np.inf], np.nan)
    base_cols = [
        "Close", "MA5", "MA10", "MA20", "MA50", "Volatility20", "RSI14",
        "Momentum5", "Momentum20", "Momentum60", "VolumeRatio20", "MACD", "MACDSignal"
    ]
    feat["TargetReturn"] = feat["Close"].shift(-1) / feat["Close"] - 1

    target_rows = feat.dropna(subset=["TargetReturn"]).copy()
    if len(target_rows) < 120:
        raise ValueError("Not enough historical data for a useful forecast yet.")

    min_obs = max(30, min(120, int(len(target_rows) * 0.20)))
    usable_cols = [c for c in base_cols if c in target_rows.columns and target_rows[c].notna().sum() >= min_obs]
    if "Close" not in usable_cols:
        raise ValueError("Price history is incomplete, so a forecast cannot be calculated.")

    medians = target_rows[usable_cols].median(numeric_only=True)
    usable_cols = [c for c in usable_cols if c in medians.index and np.isfinite(safe_float(medians[c]))]
    if "Close" not in usable_cols or len(usable_cols) < 4:
        raise ValueError("Too few usable historical features are available for a reliable forecast.")

    X = target_rows[usable_cols].copy()
    for c in usable_cols:
        X[c] = pd.to_numeric(X[c], errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(medians[c])
    X = X.fillna(0.0)
    y = pd.to_numeric(target_rows["TargetReturn"], errors="coerce")
    keep = np.isfinite(y.to_numpy(dtype=float))
    X = X.loc[keep]
    y = y.loc[keep]
    if len(X) < 120:
        raise ValueError("Not enough clean target data for a useful forecast model.")

    def new_models():
        return [
            ("Random Forest", RandomForestRegressor(
                n_estimators=220, max_depth=8, min_samples_leaf=3,
                random_state=42, n_jobs=-1
            )),
            ("Gradient Boosting", GradientBoostingRegressor(
                n_estimators=140, max_depth=3, learning_rate=.035,
                loss="huber", random_state=42
            )),
            ("Ridge", make_pipeline(StandardScaler(), Ridge(alpha=12.0))),
        ]
    ...
    for tr, te in tscv.split(X):
        fold_preds = []
        for _, model in new_models():
            try:
                model.fit(X.iloc[tr], y.iloc[tr])
                pp = np.asarray(model.predict(X.iloc[te]), dtype=float)
                if np.isfinite(pp).all():
                    fold_preds.append(pp)
            except Exception:
                continue
        if fold_preds:
            p = np.median(np.vstack(fold_preds), axis=0)
            actual = y.iloc[te].to_numpy(dtype=float)
            finite = np.isfinite(actual) & np.isfinite(p)
            if finite.any():
                maes.append(mean_absolute_error(actual[finite], p[finite]))
                dir_hits.append(np.mean(np.sign(actual[finite]) == np.sign(p[finite])))
    ...
    def statistical_step_return(history):
        close = pd.to_numeric(history["Close"], errors="coerce").dropna()
        if len(close) < 10:
            return 0.0
        daily = close.pct_change().replace([np.inf, -np.inf], np.nan).dropna()
        med_daily = safe_float(daily.tail(90).median(), 0.0)
        mom20 = safe_float(close.iloc[-1] / close.iloc[-21] - 1, 0.0) if len(close) > 21 else 0.0
        mom60 = safe_float(close.iloc[-1] / close.iloc[-61] - 1, 0.0) if len(close) > 61 else mom20
        step = 0.45 * med_daily + 0.35 * (mom20 / 20.0) + 0.20 * (mom60 / 60.0)
        return float(np.clip(step, -max_daily * .35, max_daily * .35))

    for _ in range(days):
        tmp = add_features(work).replace([np.inf, -np.inf], np.nan)
        row = tmp.iloc[-1]
        ...
        for c in usable_cols:
            ...
            if np.isnan(v):
                if c.startswith("MA"):
                    ...
                    v = 50.0
                elif c in {"Momentum5", "Momentum20", "Momentum60", "MACD", "MACDSignal"}:
                    v = 0.0
                elif c == "VolumeRatio20":
                    v = 1.0
                elif c == "Volatility20":
                    v = safe_float(medians.get(c), 0.0)
                else:
                    v = safe_float(medians.get(c), 0.0)
            row_values[c] = v

        row_df = pd.DataFrame([row_values], columns=usable_cols).replace([np.inf, -np.inf], np.nan)
        for c in usable_cols:
            row_df[c] = pd.to_numeric(row_df[c], errors="coerce").fillna(safe_float(medians.get(c), 0.0))
        row_df = row_df.fillna(0.0)

        indiv = []
        for _, model in fitted_models:
            try:
                p = float(model.predict(row_df)[0])
                if np.isfinite(p):
                    indiv.append(p)
            except Exception:
                continue
        ...
        if indiv:
            ...
            pred_ret = float(np.clip(np.median(arr), -max_daily, max_daily))
            model_disagreements.append(float(np.std(arr)) if len(arr) > 1 else 0.0)
        else:
            pred_ret = statistical_step_return(work)
            model_disagreements.append(abs(pred_ret) * .25)

        pred = last_close * (1 + pred_ret)
        nxt = last + (pd.Timedelta(days=1) if asset_type == "crypto" else pd.tseries.offsets.BDay(1))
        synthetic = pd.DataFrame(
            {"Open": [pred], "High": [pred], "Low": [pred], "Close": [pred], "Volume": [typical_vol]},
            index=[nxt],
        )
    ...
    realized_daily = safe_float(model_hist["Close"].pct_change().tail(90).std(), .03 if asset_type == "crypto" else .015)
    return_mae = float(np.mean(maes)) if maes else max(realized_daily * .75, .01)
    dir_acc = float(np.mean(dir_hits)) if dir_hits else 0.50
    disagreement = float(np.mean(model_disagreements)) if model_disagreements else realized_daily * .25
    return pred_df, return_mae, dir_acc, disagreement
